...
#Load the data
ctl <- readRDS("CC_2023mzl_ctl.rds")
AD <- readRDS("CC_2023mzl_AD.rds")

ctl <- updateCellChat(ctl) 
AD <- updateCellChat(AD)

object.list1 <- list(ctl = ctl, AD = AD)
...
#Plot for DR
```{r}
tiff("ADvsctl_diffInt_.tiff", width = 1800, height = 1800, res = 400)
gg2 <- netVisual_diffInteraction(cellchat1,  comparison = c(1,2),weight.scale = T, measure = "weight", title.name = "AD vs ctl")
dev.off()
```
...
##Change node color
```{r}
netVisual_diffInteraction5 <- function (object, comparison = c(1, 2), measure = c("count", "weight", "count.merged", "weight.merged"), color.use = NULL, 
                                        ...
                                        margin = 0.2, arrow.width = 1, arrow.size = 0.2) 
{
 ...
 vertex_labels <- c("Astrocytes", "Excitatory.Neurons", "Inhibitory.Neurons", "Microglia", "Oligodendrocytes", "Oligodendrocyte.Precursor")
