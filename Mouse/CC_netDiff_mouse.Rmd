...
#Load the data
vehicle <- readRDS("CC_DRsn2_vehicle.rds")
treatment <- readRDS("CC_DRsn2_treatment.rds")

vehicle <- updateCellChat(vehicle) 
treatment <- updateCellChat(treatment)

object.list1 <- list(vehicle = vehicle, treatment = treatment)

cellchat1 <- mergeCellChat(object.list1, add.names = names(object.list1),cell.prefix = TRUE)
...
#Plot for DR
```{r}
tiff("treatmentvsvehicle_diffInt_.tiff", width = 1800, height = 1800, res = 400)
gg2 <- netVisual_diffInteraction(cellchat1,  comparison = c(1,2),weight.scale = T, measure = "weight", title.name = "treatment vs vehicle")
dev.off()
```


#Fct: Change node color
```{r}
netVisual_diffInteraction6 <- function (object, comparison = c(1, 2), measure = c("count", "weight", "count.merged", "weight.merged"), color.use = NULL, 
